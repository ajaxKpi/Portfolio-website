    ...
    .controller("pgArticle", ['$scope', "$routeParams", 'addVisitPage', 'blogRecord', function ($scope, $routeParams, addVisitPage, blogRecord) {
        ...
        if ($scope.records.length !== 0) {
            ...
            for (record in $scope.records) {
                if ($scope.records[record]['id'] == id) {
                    $scope.record = $scope.records[record]
                }
            }
            ...
            blogRecord.query().$promise.then(function (result) {
                ...
                prepareQueryResults(result, $scope.records), (function () {
                    for (record in $scope.records) {
                        if ($scope.records[record]['id'] == id) {
                            return $scope.record = $scope.records[record]
                        }
                    }
                })()
        ...
        $scope.sendMail = function (mail) {
            ...
            if ($scope.submitForm.$valid) {
                ...
                sendMail.send(mailContainer).then(function (response) {
                        $('#send_mail').notify($scope.mainModel['emailSuccess'],
                            {
                                position: "left center", autoHideDelay: 3000, arrowShow: false,
                                style: 'bootstrap', className: 'success'
                            })
                    },
                    function (response) {
                        $('#send_mail').notify($scope.mainModel['emailError'],
                            {
                                position: "left center", autoHideDelay: 3000, arrowShow: false,
